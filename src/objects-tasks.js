...
function mergeObjects(objects) {
  const entrys = objects.reduce((acc, curr) => {
    return acc.concat(Object.entries(curr));
  }, []);
  const resultObj = {};
...
function sellTickets(queue) {
  let banknoteCount25 = 0;
  let banknoteCount50 = 0;
  let banknote;
  ...
  for (let i = 0; i < queue.length; i += 1) {
    ...
    if (banknote === 25) {
      ...
      if (banknoteCount25 === 0) {
        return false;
      }
      banknoteCount25 -= 1;
      banknoteCount50 += 1;
    } else if (banknoteCount25 > 0 && banknoteCount50 > 0) {
      banknoteCount25 -= 1;
      banknoteCount50 -= 1;
    } else if (banknoteCount25 >= 3) {
      banknoteCount25 -= 3;
    } else {
      return false;
    }
...
function fromJSON(proto, json) {
  ...
  return Object.assign(objProto, objJSON);
}
...
class MyElementSelector {
  ...
  updateSelector(value, order, occurrenceKey) {
    if (this.order > order) throw new Error(this.error.order);
    this.occurrence[occurrenceKey] += 1;
    if (this.occurrence[occurrenceKey] > 1) throw new Error(this.error.occur);
    this.result += value;
    this.order = order;
    return this;
  }
  ...
  validateOccurrence(currentValue) {
    if (currentValue > 1) {
      throw new Error(this.error.occur);
    }
  }
}
...
const cssSelectorBuilder = {
  element(value) {
    return new MyElementSelector().element(value);
  },

  id(value) {
    return new MyElementSelector().id(value);
  },

  class(value) {
    return new MyElementSelector().class(value);
  },

  attr(value) {
    return new MyElementSelector().attr(value);
  },

  pseudoClass(value) {
    return new MyElementSelector().pseudoClass(value);
  },

  pseudoElement(value) {
    return new MyElementSelector().pseudoElement(value);
  },

  combine(selector1, combinator, selector2) {
    return new MyElementSelector().combine(selector1, combinator, selector2);
  },
};
